...
class NotificationSystem:
    def __init__(self, bot_token, google_creds_file):
        ...
        # Load sheets
        self.sheet_transaksi = self.client.open("Perhitunganku").worksheet("Sheet1")
        self.sheet_budget = self.client.open("Perhitunganku").worksheet("Sheet2")
        ...
        try:
            ...
            spreadsheet = self.client.open("Perhitunganku")
            self.sheet_users = spreadsheet.add_worksheet(title="Users", rows=1000, cols=10)
            # Add headers
            headers = ['user_id', 'username', 'notifications_enabled', 'morning_reminder', 
                      'evening_summary', 'lunch_reminder', 'budget_alerts', 'weekly_report', 
                      'last_active', 'timezone']
            self.sheet_users.update('A1:J1', [headers])
        
        # Notification queues
        self.scheduled_notifications = defaultdict(list)
...
class NotificationAPI:
    ...
    def register_user(self, user_id, username=""):
        """Register new user for notifications"""
        try:
            sheet = self.notif_system.sheet_users
            records = sheet.get_all_records()

            # Cek apakah user sudah ada
            if not any(str(user_id) == str(r["user_id"]) for r in records):
                sheet.append_row([
                    str(user_id),
                    username or "",
                    True,   # notifications_enabled default ON
                    True,   # morning_reminder
                    True,   # evening_summary
                    False,  # lunch_reminder
                    True,   # budget_alerts
                ...
                logging.info(f"✅ User {user_id} ({username}) ditambahkan ke sheet Users")
            else:
                logging.info(f"ℹ️ User {user_id} ({username}) sudah ada di sheet Users")

        except Exception as e:
            logging.error(f"❌ Gagal menambahkan user {user_id}: {e}")
